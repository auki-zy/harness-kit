# 计划：harness-kit v1（dogfood 实战 #1）
...
- 发布名 **`harness-kit`**（2026-09-03 定：通用、不带用户名、registry FREE；尽早发 0.x 占位防抢注）。
...
- 2026-09-03：从 harness-template v1(71b657d) 初始化（5a2c47f）；建 GitHub 私有远端并推送（原名 harness-init，后统一改名 **auki-zy/harness-kit**）；完成同名项目调研（`docs/references/competitor-notes.md`）；登记 v1.1 候选。
...
- 2026-09-03（v1.1 doctor 落地）：新增 `doctor` 子命令（`src/doctor.ts`：关键文件 critical / 占位符·机械配置·active plan·git 警告），6/6 测试全绿；**本地端到端验证**：消费目录本地安装 harness-kit → `init demo` 写入 32 文件 → `doctor demo` 可开工(exit 0) → `doctor empty` ❌1 关键(exit 1)；确认零运行时依赖（消费端仅装 1 包）。注：沙箱内无法全局 `npm link`（写全局目录被拒），用本地目录安装等价验证；真实 `npx harness-kit` 需发布后体验。
- 下一步：用 harness-kit 开第一个真实目标项目并回填卡点 → 发布 0.x 占位 `harness-kit`（防抢注）→ v1.1 余项（--stack、权限档位示例）。
