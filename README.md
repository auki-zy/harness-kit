...
- 发布名定为 **harness-kit**（npm 裸名 `harness-init` 已被他人占用——见 [`docs/references/competitor-notes.md`](docs/references/competitor-notes.md)）。
- 状态：v1 已实现 `init`（合并不覆盖）与 `doctor`（就绪自检），本地端到端验证通过；见当前 active plan。

## 用法

```bash
harness-kit init my-app      # 把模板快照合并进 my-app（只新增不覆盖，冲突列出）
harness-kit init . --git     # 合并 + git init
harness-kit doctor           # 自检当前(harness)目录：关键文件/占位符/机械配置/plan/git
```
